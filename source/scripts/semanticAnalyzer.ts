...
module TSC {
    ...
    export class SemanticAnalyzer {
            ...
            switch(node.value){
                ...
                case Production.IntExpr:
                    ...
                    if(node.children.length > 1){
                        this.ast.addNode(new Token(TokenType.TAddition, "Addition", null, null));
                        this.ast.addNode(node.children[0].children[0].value);
                        this.ast.ascendTree();
                ...
                case Production.BooleanExpr:
                    ...
                    if(node.children.length > 1){
                        if(node.children[2].children[0].value.value == "=="){
                            this.ast.addNode(new Token(TokenType.TEquals, "TEquals", null, null));
                        }
                        else{
                            this.ast.addNode(new Token(TokenType.TNotEquals, "TNotEquals", null, null));
                        }
         ...
         * @param node the node in tree we're starting at
         */
        public checkUsedUninit(node){
            // pointer to current position in scope tree
            let ptr = this.scopeTree.curr;
            // Check current scope
            if(ptr.value.table.hasOwnProperty(node.value.value)){
                if(ptr.value.table[node.value.value].initialized == false){
                    this.warnings.push(new ScopeWarning(WarningType.UsedBeforeInit, node.value.value, node.value.lineNumber, node.value.colNumber, node.value));
                }
                return;
            }
            // Check parent scopes
